...
class GradientDescent:
	...
	def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
		...
		"""
		# Instantiate output and best, depending on self.out_type_
		output, best = None, np.inf
		...
		for t in range(self.max_iter_):
			...
			step = eta * grad
			f.weights = f.weights - step

			curr_out = f.compute_output(X=X, y=y)
			if self.out_type_ == "average":
				output.append(f.weights)
			elif self.out_type_ == "best" and curr_out < best:
				best = curr_out
				output = f.weights
